...
const TASK_MUTATION_UPDATE = gql`
  mutation UpdateTask($updateTaskId: ID!, $data: TaskInput!) {
...
const TASK_MUTATION_DELETE = gql`
...
export default function PanelTaskDescription() {
  ...
  const [mutateTaskUpdate] = useMutation(TASK_MUTATION_UPDATE);
  const [mutateTaskDelete] = useMutation(TASK_MUTATION_DELETE);
  ...
  function deleteTask() {
    return mutateTaskDelete({
      variables: {
        deleteTaskId: params.id,
      },
    }).then(() => {
      navigate('/tasks/select');
    });
  }
              ...
              <div className='col-span-6 sm:col-span-3'>
                ...
                <div className='mt-1 relative'>
                  <Controller
                    ...
                    defaultValue={
                      data?.task
                        ? new Date(data?.task.data.attributes.taskStart)
                        : null
                    }
                    render={({ field }) => (
                      <ReactDatePicker
              ...
              <div className='col-span-6'>
                ...
                <div className='mt-1 relative'>
                  <textarea
                    ...
                    defaultValue={data?.task.data.attributes.taskDescription}
                    className={classNames(
